                                          ...
                                          CXLCCI *cci)
{
    ...
    for (i = 0; i < in->num_entries_updated; i++) {
        ...
        if (ct3d->dc.total_capacity_cmd) {
            ...
            if (!mr) {
                qemu_log("Could not get memory region from "
                         "host memory backend\n");
                return CXL_MBOX_INVALID_PA;
            }
            ...
            if (ct3d->direct_mr_enabled) {
                mr_idx = ct3d->dc.cur_direct_region_idx;
                if (mr_idx >= CXL_DC_MAX_DIRECT_MR) {
                    qemu_log("Out of direct mapping slots\n");
                    return CXL_MBOX_RESOURCES_EXHAUSTED;
                }
                ...
                if (range_contains(&range, dpa)) {
                    ...
                    if (len1) {
                        cxl_insert_extent_to_extent_list(
                            ...
                            ent_start_dpa, len1, ent->tag,
                            0, ent->rid, ent->offset,
                            (uint32_t)ent->direct_window_idx);
                        cnt_delta++;
                    }
                    if (len2) {
