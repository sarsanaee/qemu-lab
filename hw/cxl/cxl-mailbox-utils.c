                                          ...
                                          CXLCCI *cci)
{
    ...
    for (i = 0; i < in->num_entries_updated; i++) {
        ...
        if (ct3d->dc.total_capacity_cmd) {
			int mr_idx = ct3d->dc.cur_dr_region_idx;
            ...
            if (ct3d->direct_mr_enabled) {
				...
				memory_region_init_alias(dr_dc_mr,
				                         ...
				                         region_offset, ct3d->dc.dc_decoder_window.size);
				memory_region_add_subregion(
				    &fw->mr, ct3d->dc.dc_decoder_window.base - fw->base + offset,
				    dr_dc_mr);
				ct3d->dc.cur_dr_region_idx = (ct3d->dc.cur_dr_region_idx + 1) %
				    4; // for now assuming 4 extents and 4 direct mapping memory regions
        ...
        case CXL_EXTENT_SELECTION_POLICY_PRESCRIPTIVE: {
            ...
            for (i = 0; i < in->ext_count; i++) {
                ...
                group = cxl_insert_extent_to_extent_group(
                    group, NULL, NULL, ext->start_dpa, ext->len, ext->tag,
                    ext->shared_seq, 0, -1, -1);
            }
